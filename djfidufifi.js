...
window.setTimeout(function () {
  ...
  var countDown = setInterval(function () {
    ...
    if (time <= 0) {
      ...
      var currentLinkIndex = 0; // تتبع الرابط الحالي
      var pageURLs = [
        "https://up.v040v.com/p/blog-page_5.html?url=" + encodeURIComponent(urlHashDecode),
        "https://up.v040v.com/p/page_6767.html?url=" + encodeURIComponent(urlHash)
      ];
      var newURL = pageURLs[currentLinkIndex]; // ابدأ بالرابط الأول
      ...
      $("#gotolink").on("click", function () {
        ...
        if (urlHashDecode) {
          // Redirect the user to the current URL
          window.location.href = newURL;

          // التبديل إلى الرابط القادم بعد الانتهاء من الرابط الحالي
          currentLinkIndex = (currentLinkIndex + 1) % pageURLs.length;
          newURL = pageURLs[currentLinkIndex];
        } else {
          alert('hash invalid');
        }
      });
